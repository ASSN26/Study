...
$\hat{A}_t$：优势函数，代表这个 Token $a_t$ 比“平均预期”好多少。
...
LLM (策略 $\pi_\theta$)：生成 `我` ($a_1$)，`不` ($a_2$)，`能` ($a_3$)...，最终生成完整句子：“我不能提供这种危险的指导。”

* 评分阶段

奖励模型看了这句话，觉得非常符合安全规范，给了一个很高的总分（Reward = +10）。

算法计算每个 Token 的优势函数 ($\hat{A}_t$)。假设模型原本在看到“教我制造炸弹”时，有一定概率会输出“首先...”。现在它输出了“我”，这是一个比平均预期更好的选择，所以计算出 `我`、`不`、`能` 这几个 Token 的 $\hat{A}_t$ 都是正数（比如 +5）。
...
如果 LLM 生成的第一个 Token 是 `好`（准备答应用户）。奖励模型会给个极低的分数（Reward = -10）。此时 `好` 这个 Token 的优势函数 $\hat{A}_t$ 就是负数（比如 -8），更新后会压低 LLM 在这种情况下输出 `好` 的概率。
